...
def hopper():
	...
	kernel = np.ones((3,3))
	erosion = cv2.erode(img,kernel,iterations = 20) #increase if necessary 
	...
	for i in range(0,len(yCenterValues)):
		width = len(edges[0])
		height = sum([len(arr) for arr in edges])/width
		if(yCenterValues[i]>0 and yCenterValues[i]<400 and cv2.contourArea(FINALCONTOURS[i])>10000): #modify contour area later
